...
public class SimpleKeyGenerator extends KeyGenerator {
  ...
  @Override
  public HoodieKey getKey(Row row) {
    if (recordKeyField == null || partitionPathField == null) {
      throw new HoodieException(
          "Unable to find field names for record key or partition path in cfg");
    }
    return new HoodieKey(DataSourceUtils.getNestedFieldValAsString(row, recordKeyField),
        DataSourceUtils.getNestedFieldValAsString(row, partitionPathField));
  }
}
