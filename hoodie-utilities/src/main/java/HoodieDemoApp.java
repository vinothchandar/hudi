...
/**
 * Demo program that writes & reads hoodie datasets via the Spark datasource
 */
public class HoodieDemoApp {
  ...
  @Parameter(names = {"--use-multi-partition-keys", "-mp"},
  ...
  private static Logger logger = LogManager.getLogger(HoodieDemoApp.class);

  public static void main(String[] args) throws Exception {
    HoodieDemoApp cli = new HoodieDemoApp();
  ...
  public void run() throws Exception {
     ...
     */
    // Generate some input..
    List<String> records1 = Files.readLines(new File("docker/demo/data/batch_1.json"),
        StandardCharsets.UTF_8);
    Dataset<Row> inputDF1 = spark.read().json(jssc.parallelize(records1, 2));

    // Save as hoodie dataset (copy on write)
    DataFrameWriter<Row> writer = inputDF1.write()
        ...
        .option(DataSourceWriteOptions.RECORDKEY_FIELD_OPT_KEY(),
            "key") // This is the record key
        .option(DataSourceWriteOptions.PARTITIONPATH_FIELD_OPT_KEY(),
            "date") // this is the partition to place it into
     ...
     */
    List<String> records2 = Files.readLines(new File("docker/demo/data/batch_2.json"),
        StandardCharsets.UTF_8);
    Dataset<Row> inputDF2 = spark.read().json(jssc.parallelize(records2, 2));
    writer = inputDF2.write().format("com.uber.hoodie")
    ...
    hoodieROViewDF.registerTempTable("stock_ticks_cow");
    spark.sql("describe stock_ticks_cow").show();
